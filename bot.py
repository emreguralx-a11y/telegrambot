# -*- coding: utf-8 -*-
# Bot: Yahoo Finance tabanlı; 15m/1h/4h trend + günlük pivot + RSI yorum
# Çalıştır: python bot_yf_tech.py
...
def daily_report():
    try:
        conn = sqlite3.connect('users.db')
        today = datetime.now().strftime("%Y-%m-%d")
        query = f"SELECT * FROM users WHERE date_added = '{today}'"
        df = pd.read_sql_query(query, conn)
        if not df.empty:
            filename = f"yeni_kullanicilar_{today}.xlsx"
            df.to_excel(filename, index=False)
            logging.info(f"Günlük rapor oluşturuldu: {filename}")
...
def run_scheduler():
    schedule.every().day.at("19:00").do(daily_report)
    while True:
...
def build_advanced_advice(t15, t1h, t4h, rsi_d, symbol):
    ...
    elif rsi_d <= 30: rsi_text = "aşırı satım bölgesinde 📉"
...
📊 **24s Değişim**: {res['chg']:+.2f}% if res['chg'] is not None else 'N/A'
"""
...
if __name__ == "__main__":
    ...
    flask_thread = Thread(target=lambda: app.run(host='0.0.0.0', port=8080, debug=False, use_reloader=False), daemon=True)
    flask_thread.start()
